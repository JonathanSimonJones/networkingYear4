...
int main()
{
	...
	while (true)
	{
		...
		while (true)
		{
			...
			// send back message for check
			send(clientSocket, buffer, sizeof(int), 0);

			//printf("Waiting upon receiving text. \n");

			int numberOfChars = atoi(buffer);

			// wait on receiving message
			char *textBuffer = (char*)malloc(numberOfChars);
			count = recv(clientSocket, textBuffer, numberOfChars, MSG_WAITALL);

			

			//printf("End of receiving text.\n");

			// FIXME: check count value
			...
			fwrite(textBuffer, 1, count, stdout);
			free(textBuffer);	// Free after use
			printf("\n");
